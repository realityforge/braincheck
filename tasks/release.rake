...
### [v#{ENV['PRODUCT_VERSION']}](https://github.com/realityforge/braincheck/tree/v#{ENV['PRODUCT_VERSION']}) (#{ENV['RELEASE_DATE']})
[Full Changelog](https://github.com/realityforge/braincheck/compare/#{from}...v#{ENV['PRODUCT_VERSION']})
HEADER
...
[Full Changelog](https://github.com/realityforge/braincheck/compare/v#{ENV['PREVIOUS_PRODUCT_VERSION']}...v#{ENV['PRODUCT_VERSION']})
...
HEADER
    ...
    stage('GithubRelease', 'Create a Release on GitHub') do
      ...
      client.login
      client.create_release('realityforge/braincheck', tag, :name => tag, :body => changes, :draft => false, :prerelease => prerelease)

      candidates = client.list_milestones('realityforge/braincheck').select {|m| m[:title].to_s == tag}
      unless candidates.empty?
        milestone = candidates[0]
        unless milestone[:state] == 'closed'
          client.update_milestone('realityforge/braincheck', milestone[:number], :state => 'closed')
        end
      end
    end
  end
